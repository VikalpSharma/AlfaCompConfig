table 70140951 "Alfa Config. Setup"
{
    ...
    fields
    {
        ...
        field(70140921; "Finance Module"; Boolean)
        {
            Caption = 'Finance Module';
        }
        field(70140922; "Inventory Module"; Boolean)
        {
    ...
    var
        ...
        ConfigSetupStandard: Record "Config. Setup";
    ...
    begin
        if "Package File Name" <> '' then begin
            FileMgt.BLOBImportFromServerFile(TempBlob, DecompressedFileName);
            IF TempBlob.Blob.HasValue() then BEGIN
                TempBlob.Blob.CreateInStream(InStr);
                XmlDocument.ReadFrom(InStr, PackageXML);
    ...
    */
    ...
    var
        ...
        LanguageID: Text;
        DocumentAttributesCollection: XmlAttributeCollection;
        DocumentAttribute: XmlAttribute;
        DocumentNode: XmlNode;
    begin
        IF PackageXML.GetRoot(DocumentElement) THEN begin
            IF DocumentElement.HasAttributes() THEN BEGIN
                DocumentNode := DocumentElement.AsXmlNode();
                DocumentNode.AsXmlElement.Attributes.Get(ConfigXMLExchange.GetElementName(ConfigPackage.FieldName(Code)), DocumentAttribute);
                "Package Code" := DocumentAttribute.Value();
                if "Package Code" = '' then
                    Error(PackageDataNotDefinedErr, FieldCaption("Package Code"));
                DocumentNode.AsXmlElement.Attributes.Get(ConfigXMLExchange.GetElementName(ConfigPackage.FieldName("Package Name")), DocumentAttribute);
                "Package Name" := DocumentAttribute.Value();
                if "Package Name" = '' then
                    Error(PackageDataNotDefinedErr, FieldCaption("Package Name"));
                DocumentNode.AsXmlElement.Attributes.Get(ConfigXMLExchange.GetElementName(ConfigPackage.FieldName("Product Version")), DocumentAttribute);
                "Product Version" := DocumentAttribute.Value();
                DocumentNode.AsXmlElement.Attributes.Get(ConfigXMLExchange.GetElementName(ConfigPackage.FieldName("Language ID")), DocumentAttribute);
                LanguageID := DocumentAttribute.Value();
                if LanguageID <> '' then
